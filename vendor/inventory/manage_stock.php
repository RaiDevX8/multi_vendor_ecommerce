	...
	<form action="" id="stock-form">
		...
		<div class="form-group">
			...
			<select name="product_id" id="product_id" class="custom-select select2">
				...
				<?php 
				$product = $conn->query("SELECT p.*,c.name as category from `product_list` p inner join category_list c on p.category_id = c.id where p.delete_flag = 0 AND p.vendor_id = '{$_settings->userdata('id')}' ".(isset($product_id) ? " or p.id = '{$product_id}'" : "")." order by (p.`name`) asc ");
				while($row= $product->fetch_assoc()):
				?>
				<option value="<?= $row['id'] ?>" <?= isset($product_id) && $product_id == 1 ? "selected" : "" ?>><?= $row['category'].' - '.$row['name'] ?> <?= $row['status'] == 0 ? "<small>(Inactive)</small>" : "" ?> <?= $row['delete_flag'] == 1 ? "<small>(Deleted)</small>" : "" ?></option>
				<?php endwhile; ?>
			</select>
		</div>
		<?php endif; ?>
